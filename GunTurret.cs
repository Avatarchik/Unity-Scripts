...
public class GunTurret : MonoBehaviour
{
    ...
    private void Prime(Vector3 inheritedVelocity)
    {
        if (!IsReadyToFire)
            return;
        ...
        if (isSequentialFiring)
        {
            ...
            FireFromPoint(barrel, inheritedVelocity, aimDeviation);
        ...
        else
        {
            ...
            {
                FireFromPoint(barrel, inheritedVelocity, 0f);
            }
        }
    ...
    private void FireFromPoint(Transform point, Vector3 inheritedVelocity, float deviation)
    {
        Vector3 spawnPosition = point.position;
        Quaternion aimRotation = point.rotation;
        if (barrelToMuzzleFlash.ContainsKey(point))
        {
            barrelToMuzzleFlash[point].Play();
        }
        Laser laserProjectile = Instantiate(laserPrefab);
        laserProjectile.Fire(spawnPosition, aimRotation, inheritedVelocity, muzzleVelocity, deviation);
    }
